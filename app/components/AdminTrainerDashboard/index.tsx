...
const AdminTrainerDashboard = ({ user }: { user: User }) => {
  ...
  const { data: attendances, isLoading: attendancesLoading } = useSWR(
    "/api/attendance",
    fetcher
  );
    ...
    <Container maxWidth="xl">
      ...
      >
        ...
        <Select
          ...
          onChange={(event) =>
            handleActiveStatus(router, event.target.value, mutate)
          }
          displayEmpty
        >
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <AppWidgetSummary
            title="Active Now"
            total={`${users.onlineUsers ? users.onlineUsers : 0}`}
            color="info"
            icon={<PeopleIcon />}
          />
        </Grid>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <AppWidgetSummary
            title="Users"
            total={`${users.count ? users.count : 0}`}
            color="error"
            icon={<PeopleIcon />}
          />
        </Grid>
